...
`ccproxy` 是一个给 Claude Code 使用的本地 Anthropic Messages API 兼容代理。它对 Claude Code 暴露 `/v1/messages`，再把请求转换到 OpenAI/Codex 中转站常见的 `/v1/chat/completions` 格式，并把响应转换回 Claude Code 需要的 Anthropic Messages/SSE 格式。
...
```dotenv
CCPROXY_UPSTREAM_BASE_URL=https://你的中转站域名/v1
CCPROXY_UPSTREAM_API_KEY=你的中转站 API Key
CCPROXY_UPSTREAM_MODEL=你的中转站模型名
```

如果你的中转站模型名是 `gpt-4.1`，可以保持示例值。如果是其他名字，例如 `gpt-5.1-codex`，把 `CCPROXY_UPSTREAM_MODEL` 改成对应值。
...
API key 可以随便填一个本地值；如果你设置了 `CCPROXY_PROXY_API_KEY`，则 Claude Code 或 ccswitch 里填写的 key 必须与它一致。
...
CCPROXY_PORT=3456
CCPROXY_UPSTREAM_BASE_URL=https://your-codex-relay.example.com/v1
CCPROXY_UPSTREAM_API_KEY=sk-your-upstream-key
CCPROXY_UPSTREAM_MODEL=gpt-4.1
CCPROXY_PROXY_API_KEY=local-claude-code-key
```
...
这表示 `Claude Code -> ccproxy` 已经通了，失败发生在 `ccproxy -> 中转站` 的 HTTPS 证书校验。
...
```dotenv
CCPROXY_UPSTREAM_CA_BUNDLE=C:\path\to\root-or-company-ca.pem
CCPROXY_UPSTREAM_SSL_VERIFY=true
```
...
改完 `.env` 后必须重启 `ccproxy`。关闭校验会让代理无法确认上游服务器身份，不建议长期使用。

另一个容易混淆的点：如果设置了 `CCPROXY_PROXY_API_KEY=local-claude-code-key`，那么 Claude Code 的 `ANTHROPIC_AUTH_TOKEN` 或 ccswitch 里填写的 API key 也必须是 `local-claude-code-key`。本地代理 key 不需要等于中转站 API key。
...
```dotenv
CCPROXY_MODEL_MAP={"claude-sonnet-4-5-20250929":"gpt-4.1","claude-3-5-sonnet-20241022":"gpt-4.1-mini"}
```

优先级：

1. `CCPROXY_MODEL_MAP` 命中时使用映射结果
2. 否则如果设置了 `CCPROXY_UPSTREAM_MODEL`，所有请求都强制使用该模型
3. 否则如果 `CCPROXY_PASS_THROUGH_MODEL=true`，透传 Claude Code 请求里的模型名
4. 最后回退到请求中的模型名

通常建议直接设置 `CCPROXY_UPSTREAM_MODEL`，最稳。
...
```dotenv
CCPROXY_UPSTREAM_BASE_URL=https://your-relay.example.com
CCPROXY_UPSTREAM_CHAT_PATH=/openai/v1/chat/completions
```

如果你的中转站不支持 `stream_options`，保持：

```dotenv
CCPROXY_RETRY_WITHOUT_STREAM_OPTIONS=true
```
...
```dotenv
CCPROXY_MAX_TOKENS_FIELD=max_completion_tokens
```

如果上游模型不接受采样参数：

```dotenv
CCPROXY_OMIT_TEMPERATURE=true
CCPROXY_OMIT_TOP_P=true
```

## 图像生成模型

如果你的中转站支持 `gpt-image-2` 这类图像模型，不要再让它走 `/v1/chat/completions`。这类模型需要 Images API，代理会在上游模型匹配 `CCPROXY_IMAGE_MODELS` 时自动改走：
...
```dotenv
CCPROXY_UPSTREAM_MODEL=gpt-image-2
CCPROXY_UPSTREAM_IMAGES_GENERATIONS_PATH=/images/generations
CCPROXY_UPSTREAM_IMAGES_EDITS_PATH=/images/edits
CCPROXY_IMAGE_OUTPUT_DIR=generated-images
CCPROXY_IMAGE_SIZE=auto
CCPROXY_IMAGE_QUALITY=auto
CCPROXY_IMAGE_BACKGROUND=auto
CCPROXY_IMAGE_OUTPUT_FORMAT=png
CCPROXY_IMAGE_N=1
CCPROXY_IMAGE_MAX_REFERENCE_IMAGES=16
```
...
代理会提取最后一条用户文本作为图片 prompt，调用上游 Images API，把返回的 base64 图片保存到 `CCPROXY_IMAGE_OUTPUT_DIR`，并把本地文件路径返回给 Claude Code。
...
- 如果需要局域网访问，务必设置 `CCPROXY_PROXY_API_KEY`
- 不要在生产环境开启 `CCPROXY_DEBUG_PAYLOADS=true`，它会记录完整 prompt 和工具参数
...
每次 `/v1/messages` 请求都会在 `ccproxy` 控制台打印一行模型路由日志：
...
能看到第一行，说明中转站到 `ccproxy` 的流式增量已经到了；如果 Claude Code 终端仍然等完整答案才显示，多半是 Claude Code 当前 UI/调用方式在缓冲展示。若 `model route` 里显示 `stream=False`，说明 Claude Code 这次请求本身不是流式请求。
